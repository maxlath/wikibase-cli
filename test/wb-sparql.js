...
describe('wb sparql', () => {
  ...
  it('should take a JS module path', async () => {
    const { stdout } = await shellExec('./bin/wd.js sparql ./test/assets/query.js Q1 --dry')
    stdout.should.containEql('wdt:P50 wd:Q1 .')
  })

  it('should take an async JS module path', async () => {
    const { stdout } = await shellExec('./bin/wd.js sparql ./test/assets/query_async.js Q2 --dry')
  ...
  describe('meta data', () => {
    it('should take a JS module path with metadata', async () => {
      ...
      const { stdout: stdoutDry } = await shellExec('./bin/wd.js sparql ./test/assets/query_with_metadata.js Q3 --dry')
      stdoutDry.should.containEql('wdt:P50 wd:Q3 .')
      const { stdout: stdoutHelp } = await shellExec('./bin/wd.js sparql ./test/assets/query_with_metadata.js --help')
