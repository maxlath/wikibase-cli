...
describe('wb add-reference', () => {
  ...
  it('should accept a JSON path', async () => {
    const { stdout, stderr } = await wbDry('add-reference ./tests/assets/add_some_reference.json')
    stderr.should.equal('')
    const { section, action, args } = JSON.parse(stdout)
    section.should.equal('reference')
    action.should.equal('set')
  ...
  it('should accept a JS module path and arguments', async () => {
    const { stdout, stderr } = await wbDry('add-reference ./tests/assets/add_some_reference.js Q4115189-c885b63e-46f9-3f51-5736-d3ed09a58acf S001191')
    stderr.should.equal('')
    const { section, action, args } = JSON.parse(stdout)
    section.should.equal('reference')
    action.should.equal('set')
