...
describe('wb edit-entity', () => {
  it('should accept a path to a JSON file', async () => {
    const { stdout } = await wdTest('edit-entity --dry ./tests/assets/edit_data.json')
    const data = JSON.parse(stdout).args[0]
    data.should.deepEqual(editDataJson)
  })

  it('should accept a path to a JS object file', async () => {
    const { stdout } = await wdTest('edit-entity --dry ./tests/assets/edit_data.js')
    const data = JSON.parse(stdout).args[0]
    data.should.deepEqual(editDataJs)
  })

  it('should accept a path to a JS function file', async () => {
    const { stdout } = await wdTest('edit-entity --dry ./tests/assets/edit_data_function.cjs Q1 123')
    const data = JSON.parse(stdout).args[0]
  ...
  it('should accept an array of edit objects', async () => {
    const { stdout } = await wdTest('edit-entity ./tests/assets/multi_edit_data.js --maxlag 100')
  ...
  it('should not log anything when the template returns undefined', async () => {
    const { stdout, stderr } = await wdTest('edit-entity --dry ./tests/assets/template_returns_undefined.js Q1')
    stdout.should.equal('')
    stderr.should.equal('')
  })
  ...
  describe('meta data', () => {
    it('should support exporting an object with a template function and metadata', async () => {
      ...
      const { stdout } = await wdTest('edit-entity --help ./tests/assets/edit_data_function.cjs')
      stdout.should.containEql('Usage:')
      stdout.should.containEql('Add a P95228 statement')
      const { stdout: dryStdout } = await wdTest('edit-entity --dry ./tests/assets/edit_data_function.cjs Q1 123')
    ...
    it('should support adding metadata to the template function when main module export', async () => {
      ...
      templateModule.description.should.be.a.String()
      templateModule.examples.should.be.an.Array()
      const { stdout } = await wdTest('edit-entity --help ./tests/assets/edit_data_function_deprecated.cjs')
      stdout.should.containEql('Usage:')
      stdout.should.containEql('Add a P95228 statement')
      const { stdout: dryStdout } = await wdTest('edit-entity --dry ./tests/assets/edit_data_function_deprecated.cjs Q1 123')
      JSON.parse(dryStdout).args[0].id.should.equal('Q1')
    })
  })
})
