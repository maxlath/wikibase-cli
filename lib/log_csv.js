...
const logCsv = ({ entities, propsAndSubProps, header }) => {
  ...
  if (firstBatch && header) {
    csv = convertToCsv({ fields: headerFields, data: formattedEntities })
    firstBatch = false
  } else {
...
const validatePropsAndInitCsvHeaders = (propsAndSubProps, join) => {
  ...
  for (const prop of props) {
    ...
    for (const subprop of subprops) {
      subpropsCount++
      headerFields.push(`${prop}.${subprop}`)
    }
  }
