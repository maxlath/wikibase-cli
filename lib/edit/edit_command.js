...
module.exports = (section, action) => {
  ...
  if (batch) {
    ...
    .pipe(through(async function (line) {
      if (line.trim() === '') return
      this.pause()
      console.error(`processing line ${++counter}:`, line)
      ...
      const lineArgs = line[0] === '{' ? [ line ] : line.split(' ')
      await runOnce(section, action, name, lineArgs)
      this.resume()
    }))
    .on('close', () => {
      console.error(`done processing ${counter} lines`)
    })
    .on('error', console.error)
  } else {
    runOnce(section, action, name, args)
  }
}

const runOnce = (section, action, name, args) => {
