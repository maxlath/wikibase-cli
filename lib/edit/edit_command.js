...
module.exports = (section, action) => {
  ...
  if (batch) {
    if (program.instance === 'https://www.wikidata.org') {
      // Always set an edit group in batch mode
      program.editGroup = program.editGroup || Date.now()
...
// Arbitrarily requiring between 6 and 50 characters
const editGroupKeyPattern = /^\w{6,50}$/

const getWbEditConfig = () => {
  const { verbose, editGroup } = program
  if (editGroup != null && !editGroupKeyPattern.test(editGroup)) {
    throw new Error(`invalid edit group key: ${editGroup} (expected pattern: ${editGroupKeyPattern})`)
  }
  let summary = '#wikibasejs/cli'
  if (editGroup) {
