  ...
  const { type } = await prompt([
    {
      ...
      options: [ 'OAuth tokens (recommended)', 'username - password' ]
    }
  ])
  if (type === '1') {
...
const getTokenOrRetry = async wbEditConfig => {
  try {
    const res = await wbEdit(wbEditConfig).getAuthData()()
    if (res.token.length !== 42) throw new Error('invalid credentials')
    saveSuccessfulCredentials(wbEditConfig)
  } catch (err) {
    ...
    // Retry
    return chooseCredentialsType(wbEditConfig.instance)
  }
}

const saveSuccessfulCredentials = wbEditConfig => {
  const { instance } = wbEditConfig
  const { oauth, username, password } = wbEditConfig.credentials
